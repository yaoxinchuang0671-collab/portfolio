...
(function () {
  ...
  function openLightboxByOrder(order) {
    ...
    var src = '';
    if (!isTriptych) {
      src = (isGif ? staticUrl : _toCdn(img ? (img.src || img.getAttribute('data-src') || '') : ''));
    ...
    if (isTriptych) {
      ...
      // 从 window.DATA 获取 slices
      var orderIdx = parseInt(item.dataset.order || '0', 10);
      var wData = (window.DATA && window.DATA.works) ? window.DATA.works[orderIdx] : null;
  ...
  function waitImagesAndReflow() {
    ...
    imgs.forEach(function(img) {
      // 懒加载图片（有 data-src 但当前无有效 src）：不参与等待，直接算完成
      var hasRealSrc = (img.src || '').replace(window.location.origin, '').length > 0;
      var isLazy = img.hasAttribute('data-src') || img.hasAttribute('data-static');
