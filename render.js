...
(async function () {
  ...
  var ANIMATED_SET = {};
  [
    'img/小程序/01电竞俱乐部_1780440296230.webp',
    'img/小程序/024电竞_1780440300814.webp',
    'img/小程序/今彩电竞_1780440348444.webp',
    'img/小程序/四风电竞俱乐部_1780440373519.webp',
    'img/小程序/小狐白电竞_1780440380593.webp',
    'img/小程序/星禾互娱电竞_1780440396593.webp',
    'img/小程序/源初电竞_1780440403613.webp',
    'img/小程序/白苏电竞_1780440321108.webp',
    'img/小程序/肥撤电竞_1780440345982.webp',
    'img/小程序/菠萝电竞_1780440341774.webp',
    'img/小程序/落魄山电竞俱乐部_1780440362450.webp',
    'img/小程序/金凰电竞_1780440350647.webp',
    'img/小程序/饱了么电竞_1780440336841.webp',
    'img/小程序/香蕉电竞_1780440374283.webp',
    'img/小程序/魔龙电竞_1780440368311.webp'
    ...
    if (serverData) {
      ...
      if (serverData.version) {
        try {
          ...
          console.warn('[render] localStorage 版本检测失败:', e.message);
        }
      }
  ...
  function renderCategories() {
    ...
    allBtn.innerHTML = '<span class="filter-label-group"><span class="filter-label-zh">全部</span><span class="filter-label-en">ALL</span></span>';
    ...
    DATA.categories.forEach(function(cat) {
      ...
      var labelEn = cat.labelEn || '';
      var enHtml = labelEn ? '<span class="filter-label-en">' + labelEn + '</span>' : '';
      btn.innerHTML = '<span class="filter-label-group"><span class="filter-label-zh">' + cat.label + '</span>' + enHtml + '</span>';
      bar.appendChild(btn);
    });
  }
  ...
  function setupLazyLoading() {
    ...
    document.querySelectorAll('#waterfall .waterfall-img[data-src], #waterfall .triptych-slice[data-src]').forEach(function(img) {
  ...
  function renderWorks(filter) {
    ...
    DATA.works.forEach(function(w, globalIdx) {
      ...
      // ── 三联图：三个切片并排 ──
      if (w.type === 'triptych' && w.slices) {
        div.classList.add('triptych-item');
        var preview = document.createElement('div');
        preview.className = 'triptych-preview';
        ['left', 'mid', 'right'].forEach(function(pos) {
          ...
          if (isKey) {
            ...
            sImg.loading = 'eager';
            sImg.fetchPriority = 'high';
          } else {
            sImg.setAttribute('data-src', toCdnUrl(w.slices[pos]));
            sImg.setAttribute('data-local', w.slices[pos]);
            sImg.loading = 'lazy';
          }
          sImg.onerror = function() { this.onerror = null; this.src = this.dataset.local || w.slices[pos]; };
          preview.appendChild(sImg);
        });
  ...
  try {
    ...
    window._categoryConfig = {
      'cat_triptych_1780868277193': { cols: 3 }  // 抖音三联图固定3列
    };
    if (Array.isArray(DATA.categories)) {
      DATA.categories.forEach(function(c) {
        if (!window._categoryConfig[c.id]) {
    ...
    console.error('[render] 主流程异常:', e);
  } finally {
    // 无论加载/渲染是否出错，强制隐藏预加载屏，防止页面永远卡住
